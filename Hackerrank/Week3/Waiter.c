...
int* waiter(int number_count, int* number, int q, int* result_count) {
    ...
    for (int i = 0; i < q; i++) {
        ...
        int B_count = 0;
        int *next_A = malloc(A_count * sizeof(int));
        int next_A_count = 0;
        for (int j = A_count - 1; j >= 0; j--) {
            if (A[j] % primes[i] == 0) B[B_count++] = A[j];
            else next_A[next_A_count++] = A[j];
        }
        for (int j = B_count - 1; j >= 0; j--) answers[answers_idx++] = B[j];
        free(B);
        free(A);
        A = next_A;
        A_count = next_A_count;
    }
    for (int i = A_count - 1; i >= 0; i--) answers[answers_idx++] = A[i];
    free(A);
    free(primes);
    *result_count = answers_idx;
    return answers;
}
